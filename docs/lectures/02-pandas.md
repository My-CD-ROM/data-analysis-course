...
## Що таке pandas і де його використовують

pandas — бібліотека Python для роботи з табличними (і взагалі
"розміченими") даними, побудована безпосередньо над NumPy. Її створив
Вес Маккіні 2008 року у фінансовій компанії AQR Capital Management саме
для аналізу панельних даних (звідси й назва: "pandas" — від "panel
data") — тоді в Python не було зручного інструменту для того, що вже
вміли спеціалізовані статистичні пакети на кшталт R. Сьогодні pandas —
де-факто стандартний перший крок практично будь-якого аналізу даних у
Python: перш ніж малювати графік чи будувати модель, дані майже завжди
спершу завантажують, чистять і перетворюють саме через pandas.

Сфери застосування — будь-яка робота з таблицями: бізнес-аналітика й
фінанси (звіти з продажів, портфелі), біоінформатика (таблиці
експериментальних вимірювань), журналістика даних (обробка відкритих
наборів даних для розслідувань), наукові дослідження (польові й
лабораторні спостереження). У межах цього курсу — практично кожна
практика від Лекції 2 і до самого кінця працюватиме саме з
`DataFrame`, а не з "сирими" масивами NumPy напряму.
