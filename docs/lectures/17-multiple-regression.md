...
**Що таке statsmodels.** На відміну від SciPy (Лекція 10), яка дає лише
окремі статистичні функції, `statsmodels` — бібліотека, що
спеціалізується саме на повноцінних статистичних моделях і
статистичному висновку: регресія (ця лекція), аналіз часових рядів
(Лекція 21), дисперсійний аналіз (ANOVA) та інші класичні методи, кожен
— із повним апаратом перевірки гіпотез (p-value, довірчі інтервали),
який очікують дослідники в економетриці, соціальних науках і
біостатистиці. Саме тому вона органічно поєднується з апаратом Лекцій
12-15 цього курсу (довірчі інтервали, перевірка гіпотез) — на відміну
від `sklearn`, що з'явиться в Лекції 22 і орієнтований на точність
прогнозу, а не на пояснення зв'язку.
