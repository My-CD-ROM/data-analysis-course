...
**Що таке `scipy.optimize`.** Це той самий SciPy, що й `scipy.stats` у
Лекції 10, лише інший підмодуль: тут зібрані алгоритми для задач
оптимізації — від лінійного програмування (`linprog`, ця лекція) до
загальної нелінійної оптимізації (`minimize`, розділ нижче). Індустрії,
де це щоденний інструмент: логістика й ланцюги постачань (розподіл
вантажів, маршрутизація), фінанси (оптимізація портфеля за
співвідношенням ризику й дохідності), виробниче планування (той самий
приклад із меблевим цехом нижче), енергетика (розподіл навантаження
між генераторами за мінімальної вартості).
